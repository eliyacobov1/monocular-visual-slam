...
1. **Determinism everywhere**: fixed seeds, reproducible artifacts, and
   deterministic ordering for every pipeline stage.
2. **Accuracy over speed**: pose stability, loop closure fidelity, and robust
   loss design trump micro-optimizations.
3. **Control-plane resilience**: supervisory orchestration, circuit breakers,
   ordering buffers, and explicit backpressure telemetry.
4. **Optimization modularity**: block-sparse factor-graph abstractions,
   deterministic solver snapshots, and pluggable robust loss strategies.
5. **Telemetry intelligence**: drift detection, stage-level SLOs, JSON/CSV
   exports, and regression gating for CI.
6. **Roadmap-ready extensibility**: multi-camera and multi-sensor hooks with
   C++ acceleration paths.
...
- **Block-Sparse Optimization Engine**: preconditioned CG solves, robust loss
  kernels, and per-iteration diagnostics baked into solver telemetry.
